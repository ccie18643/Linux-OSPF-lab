...
#### At this point we can clearly see that out of five standard (and many non standard) LSAs only two have anything to do with link state nature of OSPF protocol. Only types 1 and 2 are being fed to the Dijkstra algorithm. Well can't really blame OSPF for that since it's main competitior ISIS does exactly the same :)
