...
#### At this point we can clearly seethat out of five standard (and many non standard) LSAs only two have anything to do with link state nature of OSPF protocol. Only lsas type 1 and 2 are being fed to the Dijkstra algorithm. Well can't really blame OSPF for that sinc it's main competitior ISIS does exactly the same :)
